  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    sliderImages.forEach(image => {
        image.addEventListener("click", function() {
            // Get the title, description, and rating from data attributes
            const title = image.getAttribute("data-title");
            const description = image.getAttribute("data-description");
            const rating = image.getAttribute("data-rating");

            // Update the content in the first-header section
            ...
            for (let i = 0; i < 5; i++) {
                ratingElement.innerHTML += `<span class="star">${i < rating ? "&#9733;" : "&#9734;"}</span>`;
            }
        });
    });
});
